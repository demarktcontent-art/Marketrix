...
import { ContentType, ContentStatus, AdPlatform, AdStatus, AdItem } from '../types';
...
export default function ProductDetail() {
  ...
  const handleAdSubmit = async (e: React.FormEvent) => {
    ...
    if (existingAd) {
      ...
      if (adForm.status === 'Live Ad') {
        newAd.startedAt = new Date().toISOString();
      }
      await addAd(newAd);
    }
                      ...
                      <div className="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                        ...
                        <div className="flex flex-col">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            ...
                            ad.status === 'Stopped' ? 'bg-red-100 text-red-800' :
                          ...
                          {ad.startedAt && (
                            <span className="text-[10px] text-green-600 mt-1">
                              Started: {new Date(ad.startedAt).toLocaleDateString()}
                            </span>
                          )}
                          {ad.stoppedAt && (
                            <span className="text-[10px] text-red-600">
                              Stopped: {new Date(ad.stoppedAt).toLocaleDateString()}
                            </span>
                          )}
                        </div>
                      </div>
